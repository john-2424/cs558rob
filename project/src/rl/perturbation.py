...
def sample_perturbation(rng, xy_range, yaw_range, z_range=0.0,
                        pitch_range=0.0, roll_range=0.0):
    dx = rng.uniform(-xy_range, xy_range)
    dy = rng.uniform(-xy_range, xy_range)
    dz = rng.uniform(-z_range, z_range) if z_range > 0 else 0.0
    droll = rng.uniform(-roll_range, roll_range) if roll_range > 0 else 0.0
    dpitch = rng.uniform(-pitch_range, pitch_range) if pitch_range > 0 else 0.0
    dyaw = rng.uniform(-yaw_range, yaw_range) if yaw_range > 0 else 0.0
...
def perturb_cube_pose(cube_id, nominal_pos, nominal_orn_euler, rng,
                      ...
                      pitch_range=0.0, roll_range=0.0):
    dx, dy, dz, droll, dpitch, dyaw = sample_perturbation(
        rng, xy_range, yaw_range, z_range,
    ...
    new_euler = [
        nominal_orn_euler[0] + droll,
        nominal_orn_euler[1] + dpitch,
    ...
    return np.array(new_pos, dtype=float), new_orn, (dx, dy, dz, droll, dpitch, dyaw)
